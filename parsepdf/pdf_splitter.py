    ...
    def start_splitting(self):
        ...
        for i in range(1, 11):
            ...
            for j in range(len(self.base_coordinates[i]) - 1):
                ...
                print(f'Cropping upperLeft: {(self.base_coordinates[i][j][0], self.base_coordinates[i][j][1] + 25)}')
                try:
                    page.cropBox.lowerRight = (self.page_size[1], self.base_coordinates[i + 1][j][1] + 25)
                    print(f'Cropping lowerRight: {(self.page_size[1], self.base_coordinates[i + 1][j][1] + 25)}')
                except IndexError:
...
# m_pdf = PDFSplitter('parsepdf/questions_coordinates.json', 'parsepdf/EuclidCombinedContest.pdf')
